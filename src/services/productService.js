...
class ProductService {
  ...
  async getProductById(id) {
    try {
      ...
      if (cachedProduct) {
        const duration = Date.now() - startTime;
        console.info(`Product ${id} retrieved from cache in ${duration}ms`);
        return {
      ...
      const duration = Date.now() - startTime;
      console.info(`Product ${id} retrieved from database in ${duration}ms`);
  ...
  async createProduct(productData) {
    try {
      ...
      const duration = Date.now() - startTime;
      console.info(`Product created in ${duration}ms`);
      return {
  ...
  async getCategories() {
    try {
      ...
      if (cachedCategories) {
        const duration = Date.now() - startTime;
        console.info(`Categories retrieved from cache in ${duration}ms`);
        return {
